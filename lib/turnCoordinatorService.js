...
function recordEvent(options = {}, event = {}) {
  ...
  try {
    const result = recorder.recordEvent({
      ...
      source_type: options.sourceType || options.source_type || event.source_type,
      ...event,
    });
...
function utteranceFor(payload = {}) {
  return normalizeText(payload.user_goal)
    || normalizeText(payload.utterance)
    || normalizeText(payload.spoken_context)
    || normalizeText(payload.conversation_summary)
    || "";
}

function detectUtteranceSource(payload = {}) {
  for (const field of ["user_goal", "utterance", "spoken_context", "conversation_summary"]) {
    if (normalizeText(payload[field])) return field;
  }
  return null;
}

function logInvalidToolArguments(payload, utteranceSource, options) {
  const providedFields = payload && typeof payload === "object" && !Array.isArray(payload)
    ? Object.keys(payload).sort()
    : [];

  recordEvent(options, {
    category: "frontend",
    action: "invalid_tool_arguments",
    status: "warning",
    summary: "coordinate_reasoning_turn received incomplete Realtime tool arguments.",
    payload: {
      tool_name: "coordinate_reasoning_turn",
      provided_fields: providedFields,
      recovered_from: utteranceSource,
      expected_any_of: ["user_goal", "utterance", "spoken_context", "conversation_summary"],
    },
  });
}
...
async function coordinateReasoningTurn(payload = {}, state, options = {}) {
  ...
  const utteranceSource = detectUtteranceSource(payload);
  const utterance = utteranceFor(payload);
  if (utteranceSource !== "user_goal" && utteranceSource !== "utterance" && utteranceSource !== "spoken_context") {
    logInvalidToolArguments(payload, utteranceSource, options);
  }
  if (!utterance) {
    return coordinateClarify({
      needs_clarification: CLARIFY_FALLBACK,
    }, state);
  }
