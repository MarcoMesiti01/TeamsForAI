...
function resolveTurnId(operation, metadata) {
  const operationTurnId = validateOptionalTurnId(
    operation.source_turn_id,
    "Workspace operation source_turn_id"
  );
  const metadataTurnId = validateOptionalTurnId(
    metadata.turn_id,
    "Workspace metadata turn_id"
  );
...
function applyWorkingMemoryUpdate(workspace, operation, appliedAt) {
  ...
  if (Object.keys(changes).length === 0) {
    throw new Error("Working memory update must include at least one recognized field.");
  }
