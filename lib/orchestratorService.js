...
const DEFAULT_ORCHESTRATOR_MODEL = process.env.ORCHESTRATOR_MODEL || "gpt-4.1-mini";
...
function fallbackDecision(input, reason) {
  ...
  return {
    ...
    confidence: 0.35,
    required_context: [],
    preferred_model: DEFAULT_ORCHESTRATOR_MODEL,
    tool_plan: [],
  };
}
...
function normalizeDecision(rawDecision, input) {
  ...
  return {
    ...
    preferred_model: normalizeText(decision.preferred_model) || DEFAULT_ORCHESTRATOR_MODEL,
...
async function callOrchestratorModel(input, options = {}) {
  ...
  const model = options.model || DEFAULT_ORCHESTRATOR_MODEL;
