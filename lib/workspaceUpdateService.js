...
const UNDO_REQUEST_RE = /\b(?:undo|revert|roll\s+back)\s+(?:the\s+)?(?:last|previous|recent|that\s+last)\s+(?:reasoning(?:\s+update)?|memory(?:\s+update)?|decision|conclusion)\b/i;
...
function normalizeUpdate(raw = {}, workspace) {
  ...
  const malformedUpdateOperations = action === "update" && !Array.isArray(output.operations);
  const validOperations = operations !== null
    && !malformedUpdateOperations
    && operationsAreWorkspaceValid(operations, workspace);

  return {
    action: validOperations ? action : "clarify",
    operations: validOperations ? operations : [],
...
async function proposeWorkspaceUpdate(payload = {}, workspace, options = {}) {
  const utterance = [payload.utterance, payload.user_goal]
    .map(normalizeText)
    .find(Boolean) || "";
  if (!utterance) {
