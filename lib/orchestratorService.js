...
function normalizeToolPlan(value) {
  ...
  return value.map((step) => ({
    ...
    args: step.args && typeof step.args === "object"
      ? step.args
      : tryParseArgsJson(step.args_json),
    reason: normalizeText(step.reason),
  })).filter((step) => step.tool);
}

function tryParseArgsJson(value) {
  const text = normalizeText(value);
  if (!text) return {};
  try {
...
function buildToolPlanArgsSchema() {
  return {
    type: "object",
    properties: {
      artifact_type: { type: "string" },
      board_strategy: { type: "string" },
...
async function callOrchestratorModel(input, options = {}) {
  recordEvent(options, {
    ...
    action: "orchestrator",
    status: "started",
    summary: "calling orchestrator model",
    payload: {
  ...
  if (!apiKey) {
    recordEvent(options, {
      ...
      action: "orchestrator",
      status: "failed",
      summary: "orchestrator model unavailable",
      payload: {
        orchestrator_input: input,
    ...
    recordEvent(options, {
      ...
      status: "completed",
      summary: "orchestrator used deterministic fallback",
      payload: {
        orchestrator_input: input,
        reason: "Missing OPENAI_API_KEY",
      },
  ...
  if (!response.ok) {
    recordEvent(options, {
      ...
      action: "orchestrator",
      status: "failed",
      summary: "orchestrator model request failed",
      payload: {
        orchestrator_input: input,
        error: data?.error?.message || "Orchestrator model request failed",
      },
    });
    throw new Error(data?.error?.message || "Orchestrator model request failed");
  }

  const normalizedOutput = normalizeDecision(parseModelJson(data), input);
  recordEvent(options, {
    category: "model",
    action: "orchestrator",
    status: "completed",
    summary: "orchestrator model completed",
    payload: {
      orchestrator_input: input,
...
async function createOrchestratorDecision(payload = {}, options = {}) {
  ...
  if (obviousFallback.intent_type === "undo") {
    recordEvent(options, {
      category: "model",
      action: "orchestrator_fallback",
      status: "completed",
      summary: "orchestrator used explicit undo fallback",
      payload: {
        orchestrator_input: input,
        fallback_output: obviousFallback,
      },
    });
    return obviousFallback;
  }
  ...
  try {
    ...
    recordEvent(options, {
      ...
      action: "orchestrator_fallback",
      status: "completed",
      summary: "orchestrator fell back after model failure",
      payload: {
        orchestrator_input: input,
        error: error.message,
      },
