...
const VOICE_SAFE_FALLBACK_RESPONSE = Object.freeze({
...
function normalizeResponse(output = {}) {
  ...
  const spokenSummary = normalizeText(raw.spoken_summary);

  if (!spokenSummary) {
    return clone(VOICE_SAFE_FALLBACK_RESPONSE);
  }
  ...
  return {
    ...
    uncertainties: Array.isArray(raw.uncertainties)
      ? raw.uncertainties.map((item) => String(item).trim())
      : [],
