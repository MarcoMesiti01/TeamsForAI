...
function validateEntryFields(operation) {
  if (!CATEGORIES.has(operation.category)) {
...
function createEntry(operation, id, metadata) {
  return {
    id,
    category: operation.category,
    content: String(operation.content).trim(),
    origin: operation.origin,
    source_turn_id: operation.source_turn_id || metadata.turn_id || null,
    status: "active",
    supersedes_id: null,
  };
}
...
function applyOperation(workspace, operation, metadata, appliedAt) {
  ...
  if (operation.type === "add_entry") {
    ...
    const id = operation.id || makeId("entry");
    ensureUniqueEntryId(workspace, id);
    workspace.entries.push(createEntry(operation, id, metadata));
    return;
  }
  ...
  validateEntryFields(operation);
  const replacementId = operation.replacement_id || makeId("entry");
  ensureUniqueEntryId(workspace, replacementId);
  priorEntry.status = operation.type === "correct_entry" ? "corrected" : "superseded";
  const replacement = createEntry(operation, replacementId, metadata);
...
function applyWorkspaceOperations(workspace, operations, metadata = {}) {
  ...
  operations.forEach((operation) => {
    ...
    applyOperation(draft, operation, metadata, appliedAt);
    draft.version += 1;
