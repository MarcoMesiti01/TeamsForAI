...
const MEMORY_STATUSES = new Set(["committed", "exploratory"]);
...
function normalizeMemoryStatus(value) {
  return MEMORY_STATUSES.has(value) ? value : "exploratory";
}

function sanitizeProvenanceFields(operation) {
  const fields = {};
  if ("workspace_entry_id" in operation) {
    fields.workspace_entry_id = normalizeOptionalText(operation.workspace_entry_id);
  }
  if ("memory_status" in operation) {
    fields.memory_status = normalizeMemoryStatus(operation.memory_status);
  }
  if ("origin" in operation) {
    fields.origin = normalizeOptionalText(operation.origin);
  }
  return fields;
}

function sanitizeBoardOperation(operation) {
  if (operation.type === "create_node") {
    return {
      ...operation,
      workspace_entry_id: normalizeOptionalText(operation.workspace_entry_id),
      memory_status: normalizeMemoryStatus(operation.memory_status),
      origin: normalizeOptionalText(operation.origin),
    };
  }
  if (operation.type === "update_node") {
    return {
      ...operation,
      ...sanitizeProvenanceFields(operation),
    };
  }
  return operation;
}
...
function applyOperation(board, operation) {
  ...
  if (operation.type === "create_node") {
    upsertById(board.nodes, {
      ...
      workspace_entry_id: operation.workspace_entry_id,
...
function applyBoardOperations(board, operations = [], metadata = {}) {
  ...
  operations.forEach((operation) => {
    const sanitizedOperation = sanitizeBoardOperation(operation);
    applyOperation(board, sanitizedOperation);
    board.version += 1;
    board.operation_log.push({
      ...clone(sanitizedOperation),
