...
function workspaceVersionMismatch(sessionState, command) {
  if (!command || !Number.isFinite(command.expected_workspace_version)) return false;
  const currentVersion = sessionState.workspace?.version;
  return Number.isFinite(currentVersion) && currentVersion !== command.expected_workspace_version;
}

function markJobFailed(job, error, sessionState) {
  job.status = "failed";
...
async function runWhiteboardJob(sessionState, jobId, options = {}) {
  ...
  try {
    ...
    if ((plan.warnings || []).length && options.plannerOptions?.disableFallback) {
      markJobFailed(job, `Planner returned invalid operations: ${plan.warnings.join(" ")}`, sessionState);
      recordEvent(options, {
        category: "whiteboard_job",
        action: "failed",
        status: "failed",
    ...
    if (workspaceVersionMismatch(sessionState, job.command)) {
      markJobFailed(
        job,
        `Skipped stale workspace board sync: expected workspace version ${job.command.expected_workspace_version}, current version ${sessionState.workspace.version}.`,
        sessionState
      );
      recordEvent(options, {
        category: "whiteboard_job",
